...
6. **Allgemeine Helfer** — `nichtNegativ` (max 0), `hoechstens` (min /
   ...
   („je angefangene Einheit" = `aufrunden(wert / teiler)`). Aus den
...
| **Rundung** | Builtins: `abrundenEuro/aufrundenEuro(EuroCent)→Euro`, `ab/aufrundenCent(EuroCent)→Cent`, `abrunden/aufrunden(Dezimal)→Ganzzahl`. „Auf volle 100 € ab": `(abrunden(betrag / 100) * 100) als EuroCent`. „Je angefangene Einheit": `aufrunden(wert / teiler)`. |
