...
/**
 * Sprechender Domänen-Sicht-Typ `Euro` — eine {@link FinDslNumber}
 ...
 * Laufzeit-Tag bleibt im geerbten {@code type}-Feld; {@code Euro} ist
 * nur die an Deklarationsgrenzen (fn-Param/-Rückgabe, {@code record}-
 * Feld, {@code konst}) sichtbare Sicht. Arithmetik auf einem {@code Euro}
 ...
 */
public final class Euro extends FinDslNumber {

    /** Paket-privat: Instanzen entstehen über {@link #von}. */
    Euro(BigDecimal value, Type type) {
        super(value, type);
    }
     ...
     * auch wenn die Sicht {@code Euro} ist). Die Methode prüft den Tag
     ...
     * <i>übergebenen Ausdruck</i> ({@code .cast(Type.Euro)} /
     * {@code .withMoneyAnnotation(Type.Euro,…)} /
     * {@code .abrunden(Type.Euro)}), 1:1 gespiegelt vom Interpreter
     ...
     *
     * <p>Konsequenz: der Java-Typ {@code Euro} ist ein API-Sicht-Etikett
     ...
     * @return die {@code Euro}-Sicht desselben Werts/Tags.
     */
    public static Euro von(FinDslNumber kern) {
        return new Euro(kern.value(), kern.type());
    }

    /**
     * Sprechender Konstruktions-Helfer — delegiert an die bit-genaue
     * Factory {@link FinDslNumber#euro(String)} (Tag {@link Type#Euro}).
     * Im Unterschied zu {@link #von(FinDslNumber)} <i>erzeugt</i> dies
     * einen Wert (mit Tag {@link Type#Euro}), statt einen vorhandenen
     * nur zu etikettieren.
     *
     * @param wert Dezimalstring (Punkt als Dezimaltrenner).
     * @return ein {@code Euro} mit dem geparsten Wert.
     */
    public static Euro von(String wert) {
        return von(FinDslNumber.euro(wert));
    }
}
