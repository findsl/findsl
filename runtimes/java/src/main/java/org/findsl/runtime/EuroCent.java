...
/**
 * Sprechender Domänen-Sicht-Typ `EuroCent` — eine {@link FinDslNumber}
 ...
 * Laufzeit-Tag bleibt im geerbten {@code type}-Feld; {@code EuroCent} ist
 * nur die an Deklarationsgrenzen (fn-Param/-Rückgabe, {@code record}-
 * Feld, {@code konst}) sichtbare Sicht. Arithmetik auf einem {@code EuroCent}
 ...
 */
public final class EuroCent extends FinDslNumber {

    /** Paket-privat: Instanzen entstehen über {@link #von}. */
    EuroCent(BigDecimal value, Type type) {
        super(value, type);
    }
     ...
     * {@code EuroCent} <b>getypte</b> Instanz.
     *
     ...
     * auch wenn die Sicht {@code EuroCent} ist). Die Methode prüft den Tag
     ...
     * <i>übergebenen Ausdruck</i> ({@code .cast(Type.EuroCent)} /
     * {@code .withMoneyAnnotation(Type.EuroCent,…)} /
     * {@code .abrunden(Type.EuroCent)}), 1:1 gespiegelt vom Interpreter
     ...
     *
     * <p>Konsequenz: der Java-Typ {@code EuroCent} ist ein API-Sicht-Etikett
     ...
     * @return die {@code EuroCent}-Sicht desselben Werts/Tags.
     */
    public static EuroCent von(FinDslNumber kern) {
        return new EuroCent(kern.value(), kern.type());
    }

    /**
     * Sprechender Konstruktions-Helfer — delegiert an die bit-genaue
     * Factory {@link FinDslNumber#euroCent(String)} (Tag {@link Type#EuroCent}).
     * Im Unterschied zu {@link #von(FinDslNumber)} <i>erzeugt</i> dies
     * einen Wert (mit Tag {@link Type#EuroCent}), statt einen vorhandenen
     * nur zu etikettieren.
     *
     * @param wert Dezimalstring (Punkt als Dezimaltrenner).
     * @return ein {@code EuroCent} mit dem geparsten Wert.
     */
    public static EuroCent von(String wert) {
        return von(FinDslNumber.euroCent(wert));
    }
}
