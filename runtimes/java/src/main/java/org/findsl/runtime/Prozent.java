...
/**
 * Sprechender Domänen-Sicht-Typ `Prozent` — eine {@link FinDslNumber}
 ...
 * Laufzeit-Tag bleibt im geerbten {@code type}-Feld; {@code Prozent} ist
 * nur die an Deklarationsgrenzen (fn-Param/-Rückgabe, {@code record}-
 * Feld, {@code konst}) sichtbare Sicht. Arithmetik auf einem {@code Prozent}
 ...
 */
public final class Prozent extends FinDslNumber {

    /** Paket-privat: Instanzen entstehen über {@link #von}. */
    Prozent(BigDecimal value, Type type) {
        super(value, type);
    }
     ...
     * {@code Prozent} <b>getypte</b> Instanz.
     *
     ...
     * auch wenn die Sicht {@code Prozent} ist). Die Methode prüft den Tag
     ...
     * <i>übergebenen Ausdruck</i> ({@code .cast(Type.Prozent)} /
     * {@code .withMoneyAnnotation(Type.Prozent,…)} /
     * {@code .abrunden(Type.Prozent)}), 1:1 gespiegelt vom Interpreter
     ...
     *
     * <p>Konsequenz: der Java-Typ {@code Prozent} ist ein API-Sicht-Etikett
     ...
     * @return die {@code Prozent}-Sicht desselben Werts/Tags.
     */
    public static Prozent von(FinDslNumber kern) {
        return new Prozent(kern.value(), kern.type());
    }

    /**
     * Sprechender Konstruktions-Helfer — delegiert an die bit-genaue
     * Factory {@link FinDslNumber#prozent(String)} (Tag {@link Type#Prozent}).
     * Im Unterschied zu {@link #von(FinDslNumber)} <i>erzeugt</i> dies
     * einen Wert (mit Tag {@link Type#Prozent}), statt einen vorhandenen
     * nur zu etikettieren.
     *
     * @param wert Dezimalstring (Punkt als Dezimaltrenner).
     * @return ein {@code Prozent} mit dem geparsten Wert.
     */
    public static Prozent von(String wert) {
        return von(FinDslNumber.prozent(wert));
    }
}
